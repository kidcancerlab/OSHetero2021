...
for(i in c("S0016", "S0018xS0028", "S0024xS0029")) {
  gen_cellecta_bc_data(
    file = paste0("/gpfs0/home/gdrobertslab/lab/Counts/",
      i,
      "/outs/possorted_genome_bam.bam"),
    output = paste0("Data/", i, ".bcs"),
    samtools_module = "SAMtools")
}

os17_cx_raw <- process_ltbc(os17_cx_raw,
  cid_lt = read.table("Data/S0016.bcs", sep = "\t"),
  histogram = F,
  relative = T)

os17_tib_raw <- process_ltbc(os17_tib_raw,
  lt_loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0018xS0028/outs/lt.fq",
  cid_loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0018xS0028/outs/cid.fq",
  histogram = F,
  relative = T)

os17_lung_raw <- process_ltbc(os17_lung_raw,
  lt_loc = "/gpfs0/home2/gdrobertslab/lab/Counts/S0024xS0029/outs/lt.fq",
  cid_loc = "/gpfs0/home2/gdrobertslab/lab/Counts/S0024xS0029/outs/cid.fq",
  histogram = F,
  relative = T)
```
...
OS.list_1500 <- list(osteoblasts = osteoblasts,
                     OS17.cx = os17_cx_raw,
...
for (i in 1:length(OS.list_1500)) {
  ...
  OS.list_1500[[i]] <- ScaleData(object = OS.list_1500[[i]],
                            vars.to.regress = c("S.Score",
                                                "G2M.Score"),
                            features = rownames(x = OS.list_1500[[i]])) # Change to VariableFeatures()
...
for (i in 1:length(tibia_list)) {
  ...
  OS_list[[title]] <- merge(tibia_list[[i]],
                              y = c(lung_list[[i]]),
                              add.cell_ids = c(head(paste(tibia_list[[i]]$model,
                                                          "_",
                                                          tibia_list[[i]]$src,
                                                          sep = ""),
                                                    1),
...
for (i in 1:length(OS_list)) {
  ...
  OS_list[[i]] <- ScaleData(object = OS_list[[i]],
                            vars.to.regress = c("S.Score",
                                                "G2M.Score"),
                            features = rownames(x = OS_list[[i]])) # Change to VariableFeatures()
