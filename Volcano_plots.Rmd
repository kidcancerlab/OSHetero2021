...
for (t in names_list) {
  ...
  scresults[[t]] <-
    foreach(c = names(clusters), .packages = "magrittr") %dopar% {
      out[[c]]$results <-
        Seurat::FindMarkers(tumor, ident.1 = clusters[c]) %>%
        dplyr::arrange(-abs(avg_log2FC))
      return(out)
    }

  plots[[t]] <-
    foreach(c = names(clusters), .packages = "ggplot2") %dopar% {
...
message(paste("Total run time: ", Sys.time() - time1))
```
...
```{r savepdfs, dependson='load', cache=TRUE}
