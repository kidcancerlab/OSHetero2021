...
coord_fixed() +
  ...
  theme(legend.position = "none")
saveRDS(primary,'primary.rds')
primary<-readRDS('primary.rds')
...
# Start from this stopping point
tumor_comb <- qs::qread("PrimaryTumor/tumor_comb.qs")
...
for(i in 1:length(unique(tumor_comb$seurat_clusters))) {
  ...
  write.table(marks, file=str_c('PrimaryTumor/tumor_combmarks',p,'.tsv'), quote=FALSE, sep='\t')
}
...
for(i in 1:length(unique(tumor_comb$seurat_clusters))) {
  ...
  p=i-1
  gsea<-read.table(str_c("PrimaryTumor/tumor_combmarks",p,".tsv"), header=T)
  ...
  head(gseGO,n=10)
  #write tsv file 
  write.table(gseGO, file=str_c('PrimaryTumor/gseGO_cluster_',p,'.tsv'), row.names=FALSE, col.name=TRUE,sep="\t")
...
celladhesion<-read.csv("PrimaryTumor/Cluster0_celladhesion_GO.csv")
celladhesion_genehits<-read.csv("PrimaryTumor/Cluster0_celladhesion_genehitsonly_GO.csv")
externalencaps<-read.csv("PrimaryTumor/Cluster0_externalencapsulatingstructure_GO.csv")
externalencaps_genehits<-read.csv("PrimaryTumor/Cluster0_externalencapsulatingstructure_hitsonly_GO.csv")
StrucMolAct_genehits<-read.csv("PrimaryTumor/Cluster0_StructMolAct_GO.csv")
