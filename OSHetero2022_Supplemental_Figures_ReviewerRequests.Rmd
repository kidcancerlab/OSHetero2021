...
```{r repfig3d_process, dependson='repfig3c', eval=TRUE}
library(data.table)
#library(pheatmap)
...
Pull out the DEG between patient primary vs met, do the same for PDXs, then compare gene lists using venn diagram.
...
coord_fixed() +
  ...
  theme(legend.position = "none")
#saveRDS(comb,'Data/Supplement/PrimaryTumor/comb.rds')
#comb<-readRDS('Data/Supplement/PrimaryTumor/comb.rds')
...
# Since have not re-processed, the cluster names are the same as those used in primary
...
qs::qsave(met_comb, "Data/Supplement/PrimaryTumor/met_comb.qs")
# Start from this stopping point
met_comb <- qs::qread("Data/Supplement/PrimaryTumor/met_comb.qs")
...
# Save a stopping point - merged and harmony aligned
qs::qsave(all_tumor, "Data/Supplement/PrimaryTumor/all_tumor.qs")
# Start from this stopping point
all_tumor <- qs::qread("Data/Supplement/PrimaryTumor/all_tumor.qs")
...
save(de_genes_dir, file="Data/Supplement/de_genes_dir.RData")
```
...
# Plot
all_tumor$tissue <- all_tumor$type %>%
  stringr::str_replace("Lung Met", "Lung")
NCHOS2$tissue <- NCHOS2$src %>%
  stringr::str_replace("Tibia", "Primary")
NCHOS7$tissue <- NCHOS7$src %>%
  stringr::str_replace("Tibia", "Primary")

models <- c(all_tumor,
            NCHOS2,
            NCHOS7)
model_names <- c("Patient Tumors",
                 "NCHOS2 PDX",
                 "NCHOS7 PDX")
names(models) <- model_names

de_out <- list()
for (cl_name in model_names) {
  Idents(models[[cl_name]]) <- models[[cl_name]]$tissue
  temp_de_out <-
    FindMarkers(models[[cl_name]],
                ident.1 = "Lung",
                ident.2 = "Primary",
                min.pct = 0.1) %>%
...
for (cl_name in model_names) {
  for (direction in c("up", "down")) {
    pdx_v_patient[[direction]] <-
      ggvenn::ggvenn(de_out[[direction]],
...
#Save as PDF
pdf("figures/supplement/requests/pdx_v_patient.pdf",
    width = 5,
    height = 10)
# Put all four plots into one image
gridExtra::grid.arrange(grobs = pdx_v_patient,
...
```{r degs, eval = F}
# Load pdx
load("Data/Supplement/de_genes_dir.RData")

# Load patient DEG list between mets and primary tumors
load("Data/Supplement/patient_de_genes_dir.RData")

# Up
## Patient DEGs
A <- rownames(patient_de_genes[["up"]])

## PDX DEGs
B <- rownames(de_genes[[1]][["up"]])
C <- rownames(de_genes[[2]][["up"]])

# Down
## Patient DEGs
D <- rownames(patient_de_genes[["down"]])

## PDX DEGs
E <- rownames(de_genes[[1]][["down"]])
F <- rownames(de_genes[[2]][["down"]])

intersect_up <- intersect(intersect(A, B), C) %>%
      unique()

intersect_down <- intersect(intersect(D, E), F) %>%
      unique()
```

## Met Module scores

## Module Scores and Scatter Plots for Cell lines and PDXs
...
save(OS.list_1500, file = "Data/Supplement/PrimaryTumor/OS.list_1500_ms.RData")
```
