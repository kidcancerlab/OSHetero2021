...
for (i in 1:length(met_tumor)){
  # met_tumor[[i]] <- met_tumor[[i]] %>% FindClusters(resolution = 0.1)
...
for(i in 1:length(met_tumor)){
  met_tumor[[i]] <- met_tumor[[i]] %>% FindClusters(resolution = 0.7)
  print(DimPlot(met_tumor[[i]], pt.size = 1, label = T) +
          coord_fixed() +
          theme(legend.position = "none") +
          ggtitle(str_c(s[i], " basic clustering")))
...
pdf(file = "Figures/supplement/met/s5.pdf",
    height=10,
    width=10)
s5
dev.off()
...
pdf(file = "Figures/supplement/met/s6.pdf",
    height=10,
    width=10)
s6
dev.off()
...
pdf(file = "Figures/supplement/met/s7.pdf",
    height=10,
    width=10)
s7
dev.off()
...
pdf(file = "Figures/supplement/met/s8.pdf",
    height=10,
    width=10)
s8
dev.off()
...
# testing <- as.data.frame(met_tumor[["BC22"]]$Glycolysis1) %>% rownames_to_column(testing)
# colnames(testing) <- c("cellid", "Glycolysis")
# testing2 <- as.data.frame(met_tumor[["BC22"]]$Hypoxia1) %>% rownames_to_column()
...
combo <- list()
  ...
  for (i in 1:length(met_tumor)) {
    ...
    pdf(file = paste0("Figures/supplement/met/s7-8.pdf"),
        height=10,
        width=10)
    print(sfig)
    dev.off()
...
combo <- list()
for (j in 1:3) {
  #for (i in 1:length(OS.list_1500[["OS17.cx"]])) {
  i <- 2
...
combo <- list()
for (j in 1:2) {
  #for (i in 1:length(OS.list_1500[OS.list_1500[[OS17.cx]]) {
  i <- 2
  p <- FeatureScatter(OS.list_1500[[i]],
                      feature1 = str_c(names(gl[2]),
                                       "1"),
