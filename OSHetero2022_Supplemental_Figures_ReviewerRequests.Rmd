...
save(k7m2_all, file = "Data/k7m2_all.RData")
```

```{r k7m2_2}
load("Data/k7m2_all.RData")
set.seed(100)
...
save(k7m2, file = "Data/Supplement/k7m2_tumor.RData")
```

```{r k7m2_3}
load("Data/Supplement/k7m2_tumor.RData")
...
save(F420_all, file = "Data/Supplement/F420_all.RData")
```

```{r f420_3}
load("Data/Supplement/F420_all.RData")
...
F420 <- RunPCA(F420, pc.genes = F420@var.genes, npcs = 20) %>%
  ...
  FindNeighbors(reduction = "pca", dims = 1:20) %>%
  FindClusters(resolution = 0.1)
save(F420, file = "Data/Supplement/F420_tumor.RData")
```
...
load("Data/Supplement/F420_tumor.RData")
load("Data/Supplement/k7m2_tumor.RData")
...
for (i in 1:length(OS.list_1500)) {
    OS.list_1500[[i]] <- subset(OS.list_1500[[i]], cells = sample(Cells(OS.list_1500[[i]]), 15))
}
#may need to put all in to same seurat object and then do PCA; or pull out Counts and calc dist
#merge all objects in 1, rescale, renormalize, redo PCA,
#pull out Counts directly (GetAssayData() slot=scaled) and calc dist or pull PCA via Embeddings
ITH.score <- list()
for (i in 1:length(OS.list_1500)) {
...
for (i in 1:length(OS.list_1500)){
  as.data.frame(ITH.score[i]) %>%
    write_excel_csv(file=paste("Data/Supplement/ITH/",
                               head(OS.list_1500[[i]]$model, 1),
                               head(OS.list_1500[[i]]$src, 1),
                               "test.csv"))
  }
for (i in 1:length(OS.list_1500)){
  test <- read.csv(file=paste("Data/Supplement/ITH/",
                              head(OS.list_1500[[i]]$model, 1),
                              head(OS.list_1500[[i]]$src, 1),
                              "test.csv"))
  
  names(test) <- paste(head(OS.list_1500[[i]]$model, 1),
                       head(OS.list_1500[[i]]$src, 1))

  scores[paste(head(OS.list_1500[[i]]$model, 1),
               head(OS.list_1500[[i]]$src, 1))] <- test
  
}
raw_ith <- as.data.frame(scores,
                         col.names = c("Osteoblast", "OS-17 Culture",
                                       "OS-17 Tibia", "OS-17 Lung",
                                       "143B Culture", "142B Tibia",
                                       "143B Lung", "NCH-OS-2 Flank",
                                       "NCH-OS-2 Tibia", "NCH-OS-2 Lung",
                                       "NCH-OS-7 Flank", "NCH-OS-7 Tibia",
                                       "NCH-OS-7 Lung", "K7M2 Culture",
                                       "K7M2 Tibia", "K7M2 Lung",
                                       "F420 Culture", "F420 Tibia",
                                       "F420 Lung"),
                         optional = TRUE) %>% na.omit()
...
repfigure_3b_15 <- rplot + xlim(0.8, 2.5) + geom_text(data=stats,
                  aes(x=2.3, y=Sample),
...
write_tsv(lung_prop_tbl, "Data/repfig3d_lung_prop_tbl.tsv")

# UMAP Plots 
sample_names <- c("OS-17_TL",
                  "143B_TL",
                  "NCH-OS-2_TL",
                  "NCH-OS-7_TL")
...
for (s_obj in sample_names) {
  ...
  tibia_plot <- DimPlot(temp_obj, reduction = "umap", pt.size = 1, label = T,
                cols = c("light grey", "#E64B35FF",  "#4DBBD5FF", "#00A087FF", "#3C5488FF"), order = T) + 
                coord_fixed() + NoLegend() + NoAxes() +
                ggtitle("Tibia") + theme(plot.title = element_text(hjust = 0.5))
  ...
  lung_plot <- DimPlot(temp_obj2, reduction = "umap", pt.size = 1, label = T,
                cols = c("light grey", "#E64B35FF",  "#4DBBD5FF", "#00A087FF", "#3C5488FF"), order = T) + 
                coord_fixed() + NoLegend() + NoAxes() +
                ggtitle("Lung") + theme(plot.title = element_text(hjust = 0.5))

  combined_plot <- tibia_plot + lung_plot

  pdf(paste0("figures/supplement/k7m2_f420/repfigure_3c1_",
                s_obj,
                ".pdf"),
       width = 5,
       height = 3)
...
for (sobj in objlist) {
  ...
  repfigure_3c2 <- ggplot(tident, aes(x = tissue, y = perc, fill = cluster)) +
  ...
  plot(repfigure_3c2)
    ...
    # Save plot as PDF
  pdf(paste0("figures/supplement/k7m2_f420/repfigure_3c2_",
                head(sobj$model,1),
                head(sobj$src, 1),
                ".pdf"),
       width = 2,
...
lung_percent <-
    read_tsv("Data/repfig3d_lung_prop_tbl.tsv") %>%
    mutate(sample_order = rank(sample) * 1000 - lung_perc,
           x_label = paste0(sample, " c", cluster) %>%
                reorder(sample_order))
...
repfigure_3d <- cowplot::plot_grid(P2,
                   P1,
                   ncol = 1,
                   align = "v",
                   rel_heights = c(2, 10))
