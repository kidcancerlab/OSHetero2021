...
load("Data/Raw/os17_cx_raw.RData")

os17.cx_noccr <- NormalizeData(os17_cx_raw) %>%
...
```{r s2}
load("Data/Raw/osteoblasts.RData")
set.seed(100)
...
osteoblasts_ccr <- RunPCA(osteoblasts_ccr, pc.genes = osteoblasts_ccr@var.genes, npcs = 20) %>%
  RunUMAP(reduction = "pca", dims = 1:20) %>%
  FindNeighbors(reduction = "pca", dims = 1:20) %>%
  FindClusters(resolution = 0.1)
...
pdf("Figures/supplement/s2/sfig_s2_a2_osteoblasts_pie.pdf",
    ...
    height = 2)
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, length(unique(osteoblasts@active.ident)))))

pies <- list()
for (i in 1:length(unique(osteoblasts@active.ident))) {
  ...
  pies[[i]] <- pie
  print(pie, vp = vplayout(ceiling(i/length(unique(osteoblasts@active.ident))), i))
}
dev.off()
...
# So, methods and cutoffs used here
# are not exactly as in the final version used for the main paper figures

#Tibia
S0018 <- tenXLoadQC(path10x = "/gpfs0/home2/gdrobertslab/lab/Counts/S0018-zymo-tib/outs/filtered_feature_bc_matrix/",
                    spec = "mixHuman")
S0018 <- subset(S0018,
...
S0042 <- tenXLoadQC(path10x = "/gpfs0/home2/gdrobertslab/lab/Counts/S0042/outs/filtered_feature_bc_matrix/",
                    spec = "mixHuman")
S0042 <- subset(S0042,
                subset = nFeature_RNA > 1500 &
                  nCount_RNA < 40000 &
...
S0034 <- tenXLoadQC(path10x = "/gpfs0/home2/gdrobertslab/lab/Counts/S0034/outs/filtered_feature_bc_matrix/",
                    spec = "mixHuman")
S0034 <- subset(S0034,
                subset = nFeature_RNA > 1500 &
                  nCount_RNA < 30000 &
...
#Lung
S0024 <- tenXLoadQC(path10x = "/gpfs0/home2/gdrobertslab/lab/Counts/S0024xS0029/outs/filtered_feature_bc_matrix/",
                    spec = "mixHuman")
S0024 <- subset(S0024,
                subset = nFeature_RNA > 1500 &
                  nCount_RNA < 60000 &
...
S0041 <- tenXLoadQC(path10x = "/gpfs0/home2/gdrobertslab/lab/Counts/S0041/outs/filtered_feature_bc_matrix/",
                    spec = "mixHuman")
S0041 <- subset(S0041,
                subset = nFeature_RNA > 1500 &
                  nCount_RNA < 60000 &
...
S0055 <- tenXLoadQC(path10x = "/gpfs0/home2/gdrobertslab/lab/Counts/S0055/outs/filtered_feature_bc_matrix/",
                    spec = "mixHuman")
S0055 <- subset(S0055,
                subset = nFeature_RNA > 1500 &
                  nCount_RNA < 50000 &
...
comb <- merge(S0018, y = c(S0042,
                           S0034,
                           S0024,
                           S0041,
                           S0055), 
              add.cell.ids = c("S0018",
                               "S0042",
                               "S0034",
                               "S0024",
                               "S0041",
...
comb<- CellCycleScoring(object = comb,
                        s.features = s.genes,
                        g2m.features = g2m.genes,
                        set.ident = TRUE)
...
qs::qsave(comb, "Data/Supplement/PrimaryTumor/comb.qs")
# Start from this stopping point
comb <- qs::qread("Data/Supplement/PrimaryTumor/comb.qs")
...
# saveRDS(primary, "PrimaryTumor/primary-ccReg.rds")
# # Start from this stopping point
# primary <- readRDS("PrimaryTumor/primary-ccReg.rds")
...
coord_fixed() +
  ...
  theme(legend.position = "none")
saveRDS(primary,'Data/Supplement/PrimaryTumor/primary.rds')
primary<-readRDS('Data/Supplement/PrimaryTumor/primary.rds')
...
saveRDS(tumor,'Data/Supplement/PrimaryTumor/tumor.rds')
tumor<-readRDS('Data/Supplement/PrimaryTumor/tumor.rds')
...
saveRDS(tumor_2,'Data/Supplement/PrimaryTumor/tumor_2.rds')
tumor_2<-readRDS('Data/Supplement/PrimaryTumor/tumor_2.rds')
...
# Save a stopping point - merged and harmony aligned
qs::qsave(tumor_comb, "Data/Supplement/PrimaryTumor/tumor_comb.qs")
# Start from this stopping point
tumor_comb <- qs::qread("Data/Supplement/PrimaryTumor/tumor_comb.qs")
...
save(primarytumor, file = "Data/Supplement/PrimaryTumor/primarytumor.RData")
```

```{r s5-s6-s7-s8_scatter}
...
sfig_s11_a1 <- processLTBC(cellecta.cx.raw,
                           ...
                           col.fill = "#E64B35FF",
                           relative = T) +
  ggtitle("Replicate #2 Lineage Tag Enrichment Analysis",
          subtitle="Culture")
