...
# Create Seurat objects and perform initial QC.
# Label original source.
# Different directory than used elsewhere-double check which should use (S0016-zymo vs S0016) -Emily
...
cx.raw <- subset(cx.raw,
                 cells = sample(Cells(cx.raw),
                                2800))
tib.raw <- subset(tib.raw,
                  cells = sample(Cells(tib.raw),
                                 2800))
lung.raw <- subset(lung.raw,
                   cells = sample(Cells(lung.raw),
                                  2800))
...
### Figure 5 Supplemental Heatmap

#### Clusters DT 

```{r Fig5_os17_supplheat_dt}
os17markers <- FindAllMarkers(os17,
                              min.pct = 0.25,
                              logfc.threshold = 0.25)
...
# Show example in Rmd html file -Emily
DimPlot(lung.sub,
        reduction = "umap",
        pt.size = 1,
        label = TRUE) +
...
DimPlot(data,
        reduction = "umap",
        pt.size = 1,
        cells.highlight = WhichCells(data,
                                     idents = "Enriched"),
