...
B) Osteosarcoma cells exhibited higher ITH scores than that of osteoblasts grown
in cell culture (####p <0.001). Within model comparison, identified increase in
ITH scores as cells colonize tibia and lung microenvironments, with the
exception of NCH-OS-7 (****p <0.001). P values were adjusted using Šidák
multiple comparisons test.

```{r Fig3b}
load("Data/OS.listv3.CCR.RData")
...
```{r, eval=FALSE}
#ComplexHeatmap
# We can update this to the matching file produced by the concatenated code RMD
cx.pd <- read.delim("Data/fromSanjanaALongTimeAgo/cxpd.tsv", row.names = 1)

#remove "HALLMARK_"
#remove "_"
rownames(cx.pd) <- rownames(cx.pd) %>%
    gsub("HALLMARK_", "", .) %>%
    gsub("_", " ", .)

df <- -log10(cx.pd) %>%#log transform
    t()
...
# We should grab these value directly from the data instead of manual insertion
aka4 <- data.frame (Lpercent = c(46.577747,
                                 10.17901,
                                 39.69814,
                                 3.545104,
...
P1 <- df %>%
  ...
  mutate(Signif = pval >= (-1 * log10(0.05)),
         sample_order = str_remove(Sample, "[0-9]") %>%
           rank() * 1000 - Lpercent,
         sample_group = str_sub(Sample, 1, 1),
         Sample = str_c(str_match(Sample, "([ABCD])(.)")[, 2],
                        "  Cluster ",
                        str_match(Sample, "([ABCD])(.)")[, 3]),
         Sample = reorder(Sample, sample_order),
         Signif = str_replace(Signif, "TRUE", "p-value < 0.05") %>%
                    str_replace("FALSE", "p-value > 0.05")) %>%
  ggplot(., aes(x = Sample, y = Pathway, fill = Signif)) + 
    geom_tile() +
...
P2 <- aka4 %>%
  ...
  mutate(sample_order = str_remove(Sample, "[0-9]") %>%
         ...
         Sample = str_c(str_match(Sample, "([ABCD])(.)")[, 2],
                        ...
                        str_match(Sample, "([ABCD])(.)")[, 3]),
         Sample = reorder(Sample, sample_order),
         ID = str_replace(ID, "t143B", "143B") %>%
                str_replace("OS2", "NCH-OS-2") %>%
