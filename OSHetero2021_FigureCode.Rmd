...
```{r Fig3c, dependson='Fig3_load'}
load("Data/OSlist_CCR.RData")
...
OS17_TL <- merge(OS.list$OS17.Tibia,
                 y = c(OS.list$OS17.Lung),
...
t143b_TL <- merge(OS.list$t143B.Tibia,
                  y = c(OS.list$t143B.Lung),
...
OS2_TL <- merge(OS.list$OS2.Tibia,
                y = c(OS.list$OS2.Lung),
...
OS7_TL <- merge(OS.list$OS7.Tibia,
                y = c(OS.list$OS7.Lung),
...
for (s_obj in c(OS17_TL, t143b_TL, OS2_TL, OS7_TL)) {
    lung_prop_tbl <-
        table(Idents(s_obj), s_obj$src) %>%
        prop.table() %>%
        as.data.frame() %>%
        as_tibble() %>%
        dplyr::rename(cluster = Var1,
                      sample = Var2,
                      lung_perc = Freq) %>%
        dplyr::filter(grepl("Lung", sample)) %>%
        mutate(lung_perc = lung_perc * 100,
...
lung_percent <-
    ...
    mutate(sample = str_replace(sample, "NCHOS2", "NCH-OS-2") %>%
           str_replace("NCHOS7", "NCH-OS-7") %>%
...
cx.pd <-
    ...
    mutate(pval = -log10(pval),
           sample_order = rank(sample) * 1000 - lung_perc,
           signif = pval >= (-1 * log10(0.05)) %>%
                str_replace("TRUE", "p-value < 0.05") %>%
