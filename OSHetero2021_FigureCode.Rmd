...
**Functional, but ongoing**

Figure 5. Lung colonization bottleneck enriches for clonal populations with a common transcriptional phenotype. A) Schematic of experimental set-up.
...
```{r Fig5b}
# Load os17 object from OSHetero_Load_Process.Rmd
load("/home/gdrobertslab/rsexf001/OSHetero2021/Data/os17.RData")

# Plot the data
set.seed(100)

# Rename src metadata for plotting purposes
os17@meta.data$src <- gsub(os17@meta.data$src,
                           pattern = "OS17_",
                           replacement = "")
...
```{r Fig5c, dependson='Fig5b'}
source("/home/gdrobertslab/rsexf001/OSHetero2021/scSeurat_updated.R")
# Change active identities to cluster numbers
...
length((cx.sub$lt))
length(unique(cx.sub$lt))
(length(unique(cx.sub$lt))/length((cx.sub$lt)))*1000

length((tib.sub$lt))
length(unique(tib.sub$lt))
(length(unique(tib.sub$lt))/length((tib.sub$lt)))*1000

length((lung.sub$lt))
length(unique(lung.sub$lt))
(length(unique(lung.sub$lt))/length((lung.sub$lt)))*1000
```
...
```{r lineagetags, dependson='Fig5c'}
source("/home/gdrobertslab/rsexf001/OSHetero2021/scSeurat_updated.R")

# Extract the barcode frequency lists 
cx.lt.list <- processLTBC(cx.sub,
                          lt.loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0016/outs/lt.fq",
                          cid.loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0016/outs/cid.fq",
                          ret.list = TRUE,
                          add.meta = FALSE)
tib.lt.list <- processLTBC(tib.sub,
                           lt.loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0018xS0028/outs/lt.fq",
                           cid.loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0018xS0028/outs/cid.fq",
                           ret.list = TRUE,
                           add.meta = FALSE)
lung.lt.list <- processLTBC(lung.sub,
                            lt.loc = "/gpfs0/home2/gdrobertslab/lab/Counts/S0024xS0029/outs/lt.fq",
                            cid.loc = "/gpfs0/home2/gdrobertslab/lab/Counts/S0024xS0029/outs/cid.fq",
                            ret.list = TRUE,
                            add.meta = FALSE)
...
```{r Fig5d_1, dependson='lineagetags'}
...
## Show in Rmd html file
DimPlot(cx.sub,
        reduction = "umap",
        pt.size = 1,
        label = TRUE) +
  coord_fixed() +
  ggtitle("Culture-derived") +
  NoAxes() +
  scale_color_npg(alpha = 1)
...
for (i in 1:nrow(tib.lt.list)) {
  ...
  tib.LTcells <- append(tib.LTcells,
                        temp)
}
tib.tagged <- subset(tib.sub,
                     cells = tib.LTcells)

## Remove outlier cells in cluster 2 and 5
## Had to edit some of these cluster names (likely due to random seed changes in plotting) -Emily
...
```{r Fig5d_cx, dependson='Fig5d_1'}
# Culture
vplayout <- function(x,y) viewport(layout.pos.row = x,
                                   layout.pos.col = y)
pdf("grid_culture.pdf", width = 10, height = 10)
grid.newpage()
pushViewport(viewport(layout = grid.layout(1,4)))
...
```{r Fig5d_tibia, dependson='Fig5d_1'}
# Tibia
vplayout <- function(x,y) viewport(layout.pos.row = x,
                                   layout.pos.col = y)
pdf("grid_tibia.pdf", width = 10, height = 10)
grid.newpage()
pushViewport(viewport(layout = grid.layout(1,4)))
...
```{r Fig5d_lung, dependson='Fig5d_1'}
# Lung
vplayout <- function(x,y) viewport(layout.pos.row = x,
                                   layout.pos.col = y)
...
# Rename idents from previous chunk 
# In order to analyze enriched clones relative to the remaining tumor cells in the lung
lung.tagged <- RenameIdents(lung.tagged,
                            "0" = "Remaining Tumor Cells",
                            "1" = "Remaining Tumor Cells",
                            "3" = "Remaining Tumor Cells",
                            "6" = "Remaining Tumor Cells")

B.list <- list(OS17 = lung.tagged)
...
# Order by -log10pvalue for plotting -Emily
