...
layout_matrix <- matrix(c(1, 1, 1, 1, 2, 2, 2, 2, 5, 5, 5, 5, 5, 5,
                          1, 1, 1, 1, 2, 2, 2, 2, 5, 5, 5, 5, 5, 5,
                          1, 1, 1, 1, 2, 2, 2, 2, 5, 5, 5, 5, 5, 5,
                          1, 1, 1, 1, 2, 2, 2, 2, 5, 5, 5, 5, 5, 5,
...
for (i in seq_along(comps$ctl)) {
  effect_size <-
    overlapping::overlap(list(ctrl = raw_ith[, comps$ctl[i]],
...
figure_3b <-
    ...
    geom_text(data = stats,
              ...
              label = paste0("p=",
                            ...
                            ", eff. size=",
                            format(stats$V2, digits = 1, trim = TRUE),
                            "\n"),
              size = 1.8,
              color = "#2b2a2a")
figure_3b
...
# There are exactly two cells in lung/tibia that cluster with the culture cells and throw off the DimPlots
odd_2_cells <-
  subset(os17, subset = src == "Tibia" | src == "Lung") %>%
  Embeddings("umap") %>%
  as.data.frame() %>%
...
os17 <-
  os17[, !colnames(os17) %in% odd_2_cells]
...
cx.lt.list <- processLTBC(os17_cx.raw,
                          lt.loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0016/outs/lt.fq",
                          cid.loc = "/gpfs0/home/gdrobertslab/lab/Counts/S0016/outs/cid.fq",
                          ret.list = TRUE)
...
# This doesn't appear to do anything since those are already the only clusters
...
lt_list <- list(cx.lt.list,
                tib.lt.list,
                lung.lt.list)

sub_list <- list(cx.sub,
                 tib.sub,
                 lung.sub)
